...
* [`src/usual_excel/`](src/usual_excel/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/usual_excel/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `usual-excel` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/usual_excel/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns usual-excel.example
  (:require [usual-excel.config :as config])
